...
Your first assignment is turned in by uploading it to a repository named `<KTH_ID>-<task name>` (ex. `dmitryc-task-2`) under the `IndaPlus26` organisation. Be careful to get the spelling right.
...
`git clone git@github.com:IndaPlus26/<KTH_ID>-<task name>.git`
