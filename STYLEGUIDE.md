...
Remember to **ALWAYS** have instructions on how to run your code, if possible in a format of a line of codeblock/script we TAs can execute easily, so that it's easier for us to grade you. Treat it like we're your classmates, and that we only have your instructions on how to run the project. With rust, you don't need to put instructions on how to download rust in every task. But for other languages and projects, be sure to leave instructions or links we can use to run your stuff 🙏
...
For **Rustoholics26**, we use the `task-X` format, so be sure to name your repos accordingly.
