...
**Required features**
* Complete movesets for at least 5 pieces (except castling & en passant)
* Check
* Turn indicator (whose turn it is)
* Function that returns the current board as a [FEN](https://www.chessprogramming.org/Forsyth-Edwards_Notation)-string
...
| `pub fn make_move(&mut self, from: &str, to: &str) -> Option<GameState>`  | If the current game state is [`GameState::InProgress`] and the move is legal, move a piece from `from` to `to` and return the resulting state of the game. Otherwise, return [`None`].             |
| `pub fn make_promotion(&mut self, piece: &str) -> Option<GameState>`         | If the current game state is [`GameState::Promoting`], promote the pawn that can be promoted to `piece`.  Otherwise, return [`None`].                                                              | 
| `pub fn get_game_state(&self) -> GameState`                                   | Get the current game state.                                                                                                                                                                        |
| `pub fn get_turn(&self) -> Colour`                                            | Get the color of the side that is currently playing.                                                                                                                                               |
| `pub fn get_possible_moves(&self, position: &str) -> Vec<String>` | If a piece is standing on the tile at `position`, return all possible new positions of that piece. Don't forget the rules for check. _(optional)_ Don't forget to include en passent and castling. |
| `pub fn to_fen(&self) -> String`                                                 | Return the current game board as a [FEN](https://www.chessprogramming.org/Forsyth-Edwards_Notation)-string for easy test assertions.                                                                                                                            |

Positions are given as strings with the format (case sensitive) `"<file><rank>"`. Ex. B4

Your program should also export an enumerator `GameState` with the values:
- `InProgress`, 
- `Check`,
- `Promoting`,
- `GameOver` or _(optional)_ `Checkmate` and `DeadPosition`.
...
> Note: We don't expect proffesional looking documentation, but atleast show some effort to try to document your code
